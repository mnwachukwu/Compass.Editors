...
// grammar owns scopes, and token colors offered through 'configurationDefaults' are accepted
// into the manifest and then ignored. Writing them into the reader's own settings is the
// supported way, which is why this is a command rather than a contribution.
//
// The palette avoids red entirely. Red is what a squiggle uses, and a language that spends it
// on ordinary keywords has nothing left to say something is wrong with.

const rules = [
    // ---- What a thing is ---------------------------------------------------------------

    // The primitive types: integer, real, boolean, character, string, fraction. Their own
    // color, held apart from the types a program declares as well as from the words around
    // them, since which primitive a thing is tends to be the question being asked.
    //
    ...
    // Words that say how far something reaches, words that build a program's shape, and the
    // three spelled-out operators: public, protected, internal, shared, constant, virtual,
    // override, abstract, sealed, extends, namespace, using, import, new, as, is, base, this,
    // and 'and', 'or', 'not'.
    //
    // The operators belong here rather than with flow because they are read rather than
    // followed: 'not ready' is a thing you evaluate, where 'if' is a thing that happens.
    ...
    // Text and characters. Muted rather than vivid: a string is often the least interesting
    ...
    // A hole, and the braces around it, read as ordinary code rather than as string.
    //
    // This rule has to exist, and setting it to the plain foreground is not the same as leaving
    // it out. A hole is written inside the string rule, so 'string.quoted.double' stays on the
    // scope stack while it is scanned, and anything in there without a scope of its own — a
    // local's name, a bracket, a comma — falls back to the deepest scope that has a color,
    // which was the string's. That is what made a hole look like the text around it. Naming the
    // hole's own scope gives it something nearer to fall back to, and code inside it goes back
    // to looking like code.
    //
    // The braces themselves are plain for the same reason: they are punctuation in code, and
    // the string on either side of them is already a different color, so nothing has to be
    ...
    // The pattern after the colon is not code — it is text handed to the value — so it keeps
    ...
    // Numbers sit in the same warm family and are told from a string by being lighter, which is
    // enough: the two rarely need telling apart at a glance, and green is spoken for by
    // comments.
    {
        scope: [
    ...
    // The label in a documentation comment: '@summary:', '@yields:', or a parameter's name.
